        ...
        <table className="w-full text-sm min-w-[1400px]">
          <thead className="bg-[#0F141C] border-b border-[#1E2633]">
            <tr className="text-left">
              ...
              <th className="px-4 py-3">{sh("Phase", "phase")}</th>
              <th className="px-4 py-3">{sh("Recommendation", "predicted_class")}</th>
              <th className="px-4 py-3">{sh("Review Priority Score", "review_priority")}</th>
              ...
              <th className="px-4 py-3 text-[10px] uppercase tracking-widest font-semibold text-slate-500">Ev. Recency</th>
              <th className="px-4 py-3 text-[10px] uppercase tracking-widest font-semibold text-slate-500">Agreement</th>
              <th className="px-4 py-3 text-[10px] uppercase tracking-widest font-semibold text-slate-500">Updated</th>
              <th className="px-4 py-3 text-right text-[10px] uppercase tracking-widest font-semibold text-slate-500">Action</th>
            </tr>
          </thead>
          <tbody>
            {cases.length === 0 && (
              <tr><td colSpan={13} className="text-center py-16 text-slate-500" data-testid="empty-cases">No cases matching filters.</td></tr>
            )}
            {cases.map((c) => (
              <tr key={c.candidate_id} className="border-b border-[#151B25] last:border-b-0 row-hover cursor-pointer"
                ...
                <td className="px-4 py-3 mono font-semibold text-white">{c.candidate_id}</td>
                <td className="px-4 py-3"><span className="mono text-xs font-semibold text-blue-300 px-2 py-0.5 rounded border border-blue-500/30 bg-blue-500/10">{c.phase}</span></td>
                ...
                <td className={`px-4 py-3 text-xs font-medium ${recencyColor(c.evidence_recency)}`}>{c.evidence_recency}</td>
                <td className={`px-4 py-3 text-xs font-medium ${agreementColor(c.agreement)}`}>{c.agreement}</td>
                <td className="px-4 py-3 mono text-[11px] text-slate-500">{new Date(c.last_updated).toLocaleDateString()}</td>
